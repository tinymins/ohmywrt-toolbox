export default {
  intro: {
    headline: "Proxy-Abonnementverwaltung",
    description:
      "Erstellen und verwalten Sie Proxy-Abos, um Konfigurationsdateien in mehreren Formaten für verschiedene Clients zu generieren. Unterstützt Clash, Clash Meta, Sing-box und andere Protokollformate.",
  },
  title: "Proxy Subscriptions",
  newSubscribe: "New Subscription",
  editSubscribe: "Edit Subscription",
  confirmDelete: "Confirm Delete",
  confirmDeleteDesc:
    "Are you sure you want to delete this subscription? This action cannot be undone.",
  createSuccess: "Created successfully",
  createFailed: "Create failed",
  updateSuccess: "Updated successfully",
  updateFailed: "Update failed",
  deleteSuccess: "Deleted successfully",
  deleteFailed: "Delete failed",
  copiedToClipboard: "Copied to clipboard",
  columns: {
    creator: "Creator",
    remark: "Remark",
    nodeCount: "Nodes",
    accessCount: "Access",
    lastUpdate: "Last Update",
    actions: "Actions",
  },
  actions: {
    edit: "Edit",
    delete: "Delete",
    preview: "Preview Nodes",
    stats: "Statistics",
  },
  common: {
    cancel: "Cancel",
    confirm: "Confirm",
  },
  tabs: {
    basic: "Basic",
    subscribeUrl: "Subscribe URL",
    ruleList: "Rule List",
    group: "Proxy Groups",
    filter: "Filter",
    customConfig: "Custom Config",
    dnsConfig: "DNS Config",
    privateAccessConfig: "Private Access",
    servers: "Manual Servers",
  },
  form: {
    remark: "Remark",
    remarkPlaceholder: "Enter a remark name",
    logLevel: "Protokollstufe",
    logLevelTip:
      "Steuert Laufzeitprotokolle in generierten Konfigurationen; Aus deaktiviert die Ausgabe",
    logLevelOff: "Aus",
    logLevelError: "Fehler",
    logLevelWarn: "Warnung",
    logLevelInfo: "Info",
    logLevelDebug: "Debug",
    subscribeUrlLabel: "Subscribe URL",
    addSubscribeItem: "Add Subscribe Source",
    subscribeItemUrl: "URL",
    subscribeItemPrefix: "Prefix",
    subscribeItemPrefixTip: "Add prefix to node names from this source",
    subscribeItemRemark: "Remark",
    subscribeItemEnabled: "Enabled",
    subscribeItemDisabled: "Disabled",
    subscribeItemCacheTtlPlaceholder: "Cache TTL (minutes)",
    useSystemConfig: "Systemkonfiguration verwenden",
    ruleListLabel: "Regelliste (JSONC, Kommentare erlaubt)",
    ruleListPlaceholder: "Regelliste JSONC-Konfiguration",
    groupLabel: "Proxy-Gruppen (JSONC, Kommentare erlaubt)",
    groupPlaceholder: "Proxy-Gruppen JSONC-Konfiguration",
    filterLabel: "Filter (JSONC, Kommentare erlaubt)",
    filterPlaceholder: "Filter JSONC-Konfiguration",
    nodeFilterLabel: "Knotenfilter",
    nodeFilterAddPlaceholder: "Filterschlüsselwort eingeben",
    customConfigLabel:
      "Benutzerdefinierte Konfiguration (JSONC, Kommentare erlaubt)",
    customConfigPlaceholder: "Benutzerdefinierte Konfiguration JSONC",
    serversLabel: "Manuelle Server",
    serversPlaceholder: "Add server configs manually (JSON format)",
    authorizedUsers: "Authorized Users",
    authorizedUsersPlaceholder: "Enter user ID",
    authorizedUsersOwnerOnly: "Only the owner can modify authorized users",
    cacheTtlTooltip: "Cache expiration time",
    cacheTtlUnit: "minutes",
    fetchUaTooltip:
      "Benutzerdefinierter User-Agent. Verschiedene Abonnementanbieter benötigen möglicherweise unterschiedliche UA",
    fetchUaPlaceholder: "UA-Tarnung (Standard: clash.meta)",
    testSourceBtn: "Diese Quelle testen",
    testSourceTitle: "Quelltest-Ergebnis",
    testSourceSuccess: "{{count}} Knoten erfolgreich abgerufen",
    testSourceFail:
      "Keine Knoten gefunden. Prüfen Sie URL oder UA-Konfiguration",
    testNodeName: "Knotenname",
    testNodeType: "Typ",
    close: "Schließen",
    jsonFormatError: "JSON format error",
    dnsConfigLabel: "DNS Config",
    dnsTabShared: "Shared Config",
    dnsLocalDns: "Local DNS",
    dnsLocalDnsPort: "Local DNS Port",
    dnsFakeipEnabled: "Enable FakeIP",
    dnsFakeipIpv4Range: "FakeIP IPv4 Range",
    dnsFakeipIpv6Range: "FakeIP IPv6 Range",
    dnsFakeipTtl: "FakeIP TTL",
    dnsDnsListenPort: "DNS Listen Port",
    dnsTproxyPort: "TProxy Port",
    dnsRejectHttps: "Reject HTTPS DNS",
    dnsCnDomainLocalDns: "Use local DNS for CN domains",
    dnsClashApiPort: "Clash API Port",
    dnsClashApiSecret: "Clash API Secret",
    dnsClashApiUiPath: "Clash API UI Path",
    dnsDnsRules: "DNS Rules",
    dnsNativeHint: "Use platform native DNS configuration format",
    dnsOverrideHint: "Override default DNS configuration",
    privateAccessConfigLabel: "Private Access",
    privateAccessConfigPlaceholder: "Private access config JSONC",
    privateAccessEnabled: "Enable private access routing",
    privateConnectorEnabled: "Enabled",
    privateConnectorDisabled: "Disabled",
    privateConnectorTag: "Connector tag",
    addPrivateConnector: "Add connector",
    privateWgAddress: "Interface address",
    privateWgPrivateKey: "Private key",
    privateWgPeerAddress: "Peer endpoint",
    privateWgPeerPort: "Peer port",
    privateWgKeepalive: "Keepalive",
    privateWgPublicKey: "Peer public key",
    privateWgPresharedKey: "Preshared key",
    privateWgAllowedIps: "Peer allowed IPs",
    privateWgReuseWarning:
      "WireGuard is peer-to-peer networking. One address/key pair should not be reused by multiple clients; create separate configs for each person or device.",
    privateOutboundServer: "Server",
    privateOutboundPort: "Port",
    privateOutboundUuid: "UUID",
    privateOutboundUsername: "Username",
    privateOutboundPassword: "Password",
    privateOutboundExtra: "Advanced outbound JSON",
    privateRouteCidrs: "Route CIDRs",
    privateRouteDomains: "Route domain suffixes",
    privateDnsDomains: "Private DNS domain suffixes",
    privateDnsGlobalWarning:
      "Without private DNS domain suffixes, this server will handle all DNS queries and may break internal service resolution.",
    privateDnsServer: "Private DNS server",
    privateDnsPort: "DNS port",
  },
  links: {
    title: "Subscribe Links",
    copy: "Copy",
    open: "Open",
    debug: "Debug",
    tools: "Werkzeuge",
    clearCache: "Cache leeren",
    cacheClearedSuccess: "Abonnement-Cache gelöscht",
  },
  preview: {
    title: "Node Preview",
    totalNodes: "Total Nodes",
    noNodes: "No available nodes",
    nodeName: "Node Name",
    server: "Server",
    port: "Port",
    protocol: "Protocol",
    transport: "Transport",
    secret: "Password",
    source: "Source",
    filtered: "Filtered",
    filteredBy: "Filter Rules",
    manual: "Manual",
    expand: "Expand",
    collapse: "Collapse",
    noDetails: "Keine Details",
    clickToCopyFull: "Click to copy full config",
    unnamed: "Unnamed",
  },
  debug: {
    title: "Subscribe Debug",
    subscribeUrls: "Subscribe URLs",
    remoteSources: "Remote Sources",
    fetchingSource: "Fetching source",
    fetchDuration: "Fetch duration",
    httpStatus: "HTTP Status",
    rawResponse: "Raw Response",
    decodedResponse: "Dekodierte Antwort",
    chars: "chars",
    detectedFormat: "Detected Format",
    parsedNodes: "Parsed Nodes",
    dataSource: "Data Source",
    sourceUrl: "Source URL",
    manualServers: "Manual Servers",
    noManualServers: "No manual servers",
    serverCount: "Server Count",
    nodeMerge: "Node Merge",
    totalNodes: "Total Nodes",
    activeNodes: "Active Nodes",
    filterRules: "Filter Rules",
    filteredNodes: "Filtered Nodes",
    filteredCount: "Filtered Count",
    afterFilter: "After Filter",
    nodesAfterFilter: "Nodes After Filter",
    nodeStats: "Node Stats",
    configParsing: "Config Parsing",
    ruleProviders: "Rule Providers",
    rules: "Rules",
    proxyGroups: "Proxy Groups",
    groupConfig: "Group Config",
    customConfigRules: "Custom Config Rules",
    dnsConfig: "DNS Config",
    privateAccessConfig: "Private Access Config",
    configBuild: "Config Build",
    finalConfig: "Final Config",
    totalDuration: "Total Duration",
    cached: "Cached",
    liveFetch: "Live Fetch",
    complete: "Complete",
    error: "Error",
    matchedRule: "Matched Rule",
    traceTitle: "Node Trace",
    traceNode: "Trace Node",
    traceSelectNode: "Select Node",
    traceSearchPlaceholder: "Search node name",
    traceNodeNotFound: "Node not found",
    traceLoading: "Loading...",
    traceNodeList: "Node List",
    traceSource: "Source",
    traceSourceUrl: "Source URL",
    traceSourceIndex: "Source Index",
    traceSourceFormat: "Source Format",
    traceRawUrl: "Raw URL",
    traceRawData: "Raw Data",
    traceParse: "Parse",
    traceClashProxy: "Clash Proxy",
    traceOriginalName: "Original Name",
    traceEnrich: "Enrich",
    traceEnrichedName: "Enriched Name",
    traceFilter: "Filter",
    traceFilterRules: "Filter Rules",
    traceFilterPassed: "Filter Passed",
    traceFilterBlocked: "Filtered Out",
    traceFilteredLabel: "Filter Result",
    traceMerge: "Merge",
    tracePosition: "Position",
    traceGroupAssign: "Group Assign",
    traceMatchedRule: "Matched Rule",
    traceNoGroupAssigned: "No Group Assigned",
    traceConvert: "Convert",
    traceSingboxOutbound: "Sing-box Outbound",
    traceProvenanceTable: "Feldherkunft",
    traceConfigFragment: "Config Fragment",
    traceOutput: "Output",
    traceManual: "Manual",
    traceSkipped: "Skipped",
    entropyWarning: "Datenverlust",
    entropyWarningShort: "Warnungen",
    entropyWarningTip:
      "Dieser Knoten hat Konfigurationsfelder während der Formatkonvertierung verloren. Klicken Sie zum Verfolgen.",
    ignoredFieldsTip:
      "Dieser Knoten hat Felder, die absichtlich ignoriert wurden (nicht anwendbar auf das Zielformat). Klicken Sie zum Verfolgen.",
    lostFieldsLabel: "Bei der Konvertierung verlorene Felder:",
    ignoredFieldsLabel:
      "Absichtlich ignorierte Felder (nicht auf Zielformat anwendbar):",
    validate: "Konfigurationsvalidierung",
    validatePassed: "Konfigurationsvalidierung bestanden",
    validateFailed: "Konfigurationsvalidierung fehlgeschlagen",
    validateSkipped: "Validierung übersprungen",
    validateMethodSingbox: "sing-box Binärprüfung",
    validateMethodYaml: "YAML-Syntaxprüfung",
    ruleSets: "Regelsatz-Prüfung",
    ruleSetCount: "Regelsätze",
    ruleSetTotalRules: "Gesamtregeln",
    ruleSetErrors: "Fehler",
    ruleSetSetsUnit: "Sätze",
    ruleSetRulesUnit: "Regeln",
    ruleSetTruncated: "gekürzt",
    ruleSetBuiltinHint:
      "Integrierter Regelsatz, Binärformat, wird automatisch vom Client heruntergeladen",
    ruleSetBuiltinTag: "integriert",
    ruleSetEffectiveUrl: "Effektive URL",
    globalSearch: "Globale Suche",
    globalSearchPlaceholder: "Endkonfiguration und Regelsätze durchsuchen…",
    globalSearchNoResults: "Keine Treffer gefunden",
    globalSearchResultCount: "{{current}}/{{total}} Ergebnisse",
    globalSearchFinalConfig: "Endkonfiguration",
    globalSearchRuleSets: "Regelsätze",
    originClickHint:
      "Klicken Sie auf einen Feldnamen, um seine Herkunft anzuzeigen",
    originNoData: "— Keine Herkunftsdaten",
    originInherited: "geerbt von",
    originSourceKey: "Quellfeld",
    originContainerSources: "Zusammengesetzt aus Feldern",
    originTablePath: "Pfad",
    originTableValue: "Wert",
    originTableTransform: "Herkunft",
    originTableSource: "Quelle",
    originTableJsonObject: "JSON-Objekt",
    originTableJsonArray: "JSON-Array ({{count}})",
    originTableFromFields: "Zusammengesetzt aus: {{fields}}",
    originTransform: {
      direct: "Direkt",
      rename: "Umbenannt",
      convert: "Konvertiert",
      extract: "Extrahiert",
      generated: "Generiert",
      fallback: "Fallback",
      container: "Konfigurationsblock",
    },
    originStep: {
      core: "Kernfelder",
      tls: "TLS-Konfiguration",
      transport: "Transport",
      multiplex: "Multiplex",
      dial: "Wähloptionen",
      type: "Protokollspezifisch",
      unknown: "Konverter-intern",
    },
    originReason: {
      trojan_requires_tls: "Trojan-Protokoll erfordert standardmäßig TLS",
      hysteria2_requires_tls: "Hysteria2-Protokoll erfordert standardmäßig TLS",
      hysteria_requires_tls: "Hysteria-Protokoll erfordert standardmäßig TLS",
      tuic_requires_tls: "TUIC-Protokoll erfordert standardmäßig TLS",
      anytls_requires_tls: "AnyTLS-Protokoll erfordert standardmäßig TLS",
      fingerprint_enables_utls:
        "uTLS automatisch aktiviert bei vorhandenem client-fingerprint",
      reality_opts_present:
        "Reality automatisch aktiviert bei konfiguriertem reality-opts",
      converter_internal: "Intern vom Konverter generiert",
    },
  },
  sourceDebug: {
    title: "Abo-Quellen-Debugging",
    mode: "Cache-Modus",
    modeValue: {
      "bypass-cache": "Cache ignorieren",
      production: "Produktionsweg simulieren",
    },
    start: "Debugging starten",
    restart: "Erneut ausführen",
    requestConfig: "Anfragekonfiguration",
    cacheCheck: "Cache-Prüfung",
    requestAttempt: "Live-Anfrage {{current}} / {{total}}",
    staleFallback: "Rückfall auf abgelaufenen Cache",
    complete: "Debugging abgeschlossen",
    url: "Abo-URL",
    userAgent: "User-Agent",
    prefix: "Knotenpräfix",
    cacheTtl: "Cache-TTL",
    maxAttempts: "Maximale Versuche",
    timeout: "Zeitlimit pro Versuch",
    cacheStatusLabel: "Cache-Status",
    cacheStatus: {
      skipped: "Übersprungen",
      miss: "Nicht gefunden",
      expired: "Gültiger Cache abgelaufen",
      hit: "Gültiger Cache gefunden",
      unusable: "Cache konnte nicht geparst werden",
    },
    fallbackStatus: {
      hit: "Letzte gültige Cache-Antwort wird verwendet",
      miss: "Kein abgelaufener Cache verfügbar",
      unusable: "Abgelaufener Cache konnte nicht geparst werden",
    },
    requesting: "Upstream-Abo-Quelle wird angefragt",
    result: "Ergebnis",
    success: "Erfolgreich",
    failed: "Fehlgeschlagen",
    httpStatus: "HTTP-Status",
    finalUrl: "Endgültige URL",
    duration: "Dauer",
    networkDiagnostics: "Netzwerkdiagnose",
    targetHost: "Zielhost",
    scheme: "Schema",
    dnsDuration: "DNS-Dauer",
    resolvedAddresses: "Aufgelöste Adressen",
    dnsError: "DNS-Fehler",
    resolverConfig: "System-DNS-Konfiguration",
    proxyEnvironment: "Proxy-Umgebungsvariablen",
    tcpProbes: "Direkte TCP-Prüfungen",
    address: "Adresse",
    localAddress: "Lokale Adresse",
    remoteAddress: "Tatsächliche Remote-Adresse",
    httpVersion: "HTTP-Version",
    tlsCertificate: "TLS-Peer-Zertifikat",
    requestErrorDetails: "Low-Level-Anfragefehlerkette",
    rawError: "Vollständiger Debug-Fehler",
    error: "Fehler",
    none: "Keine",
    detectedFormat: "Erkanntes Format",
    bodySize: "Antwortgröße",
    parsedNodes: "Gültige Knoten",
    discardedNodes: "Verworfene Platzhalterknoten",
    parseDiagnostics: "Parse-Diagnose",
    responseHeaders: "Antwort-Header",
    rawResponse: "Rohantwort",
    decodedResponse: "Dekodierter Base64-Inhalt",
    nodes: "Geparste Knoten",
    resultSource: "Ergebnisquelle",
    resultSourceValue: {
      cache: "Gültiger Cache",
      live: "Live-Anfrage",
      "stale-cache": "Abgelaufener Cache",
    },
    streamEnded: "Der Debug-Stream endete vor dem Abschlussereignis",
  },
};
