...
fn parse_url_fragment(url: &url::Url) -> String {
    url.fragment().map(url_decode).unwrap_or_default()
}
...
pub fn parse_vless_uri(uri: &str) -> Option<ClashProxy> {
    ...
    let network = params
        .get("type")
        .map_or("tcp", std::string::String::as_str);
    if network != "tcp" {
    ...
    if network == "ws" {
        ...
        let path = params
            .get("path")
            .map_or_else(|| "/".into(), |p| url_decode(p));
...
pub fn parse_vmess_uri(uri: &str) -> Option<ClashProxy> {
        ...
        && let Ok(config) = serde_json::from_str::<serde_json::Value>(&decoded)
    {
        ...
        let name = obj
            ...
            .or(obj.get("remarks"))
            .and_then(|v| v.as_str())
            .map_or_else(|| format!("{add}:{port}"), std::string::ToString::to_string);
        ...
        let cipher = obj.get("scy").and_then(|v| v.as_str()).unwrap_or("auto");
