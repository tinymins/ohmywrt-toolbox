...
fn add_core_origins(o: &mut Map<String, Value>, proxy: &ClashProxy, outbound: &Value) {
    o.insert(
        "type".into(),
        mapped_val("type", &json!(proxy.proxy_type), "core", "direct"),
    );
    o.insert(
        "tag".into(),
        mapped_val("name", &json!(proxy.name), "core", "rename"),
    );
    o.insert(
        "server".into(),
        mapped_val("server", &json!(proxy.server), "core", "direct"),
    );
    o.insert(
        "server_port".into(),
        mapped_val("port", &json!(proxy.port), "core", "rename"),
    );

    if outbound.get("network").is_some() {
        o.insert(
            "network".into(),
            mapped_val("udp", &json!(false), "core", "convert"),
        );
    }
}
...
fn add_tls_origins(o: &mut Map<String, Value>, proxy: &ClashProxy, outbound: &Value) {
    ...
    if force_tls {
        o.insert(
            "tls.enabled".into(),
            generated("tls", &format!("{pt}_requires_tls")),
        );
    ...
    if let Some(Value::Object(tls_obj)) = outbound.get("tls") {
        ...
        if tls_obj.contains_key("insecure") {
            o.insert(
                "tls.insecure".into(),
                mapped("skip-cert-verify", "tls", "rename"),
            );
        }
        ...
        if tls_obj.contains_key("utls") {
            ...
            o.insert(
                "tls.utls.enabled".into(),
                generated("tls", "fingerprint_enables_utls"),
            );
        ...
        if tls_obj.contains_key("reality") {
            ...
            o.insert(
                "tls.reality.enabled".into(),
                generated("tls", "reality_opts_present"),
            );
    ...
    match source_key {
        "ws-opts" => {
            if transport.contains_key("path") {
                o.insert(
                    "transport.path".into(),
                    mapped("ws-opts.path", "transport", "extract"),
                );
            }
            if transport.contains_key("headers") {
                o.insert(
                    "transport.headers".into(),
                    mapped("ws-opts.headers", "transport", "extract"),
                );
            }
        ...
        "http-opts" => {
            if transport.contains_key("path") {
                o.insert(
                    "transport.path".into(),
                    mapped("http-opts.path", "transport", "extract"),
                );
            }
            if transport.contains_key("method") {
                o.insert(
                    "transport.method".into(),
                    mapped("http-opts.method", "transport", "extract"),
                );
            }
            if transport.contains_key("headers")
        ...
        "h2-opts" => {
            if transport.contains_key("host") {
                o.insert(
                    "transport.host".into(),
                    mapped("h2-opts.host", "transport", "extract"),
                );
            }
            if transport.contains_key("path") {
                o.insert(
                    "transport.path".into(),
                    mapped("h2-opts.path", "transport", "extract"),
                );
            }
        }
        _ => {}
...
fn add_multiplex_origins(o: &mut Map<String, Value>, proxy: &ClashProxy) {
    ...
    o.insert(
        "multiplex.enabled".into(),
        mapped(&format!("{source_key}.enabled"), "multiplex", "extract"),
    );
    o.insert(
        "multiplex.protocol".into(),
        mapped(&format!("{source_key}.protocol"), "multiplex", "extract"),
    );
    o.insert(
        "multiplex.max_connections".into(),
        mapped(
            &format!("{source_key}.max-connections"),
            "multiplex",
            "extract",
        ),
    );
    ...
    o.insert(
        "multiplex.padding".into(),
        mapped(&format!("{source_key}.padding"), "multiplex", "extract"),
    );
}
...
fn add_vmess_origins(o: &mut Map<String, Value>, proxy: &ClashProxy) {
    ...
    o.insert(
        "security".into(),
        mapped_val("cipher", &json!(cipher), "type", "rename"),
    );
    let alt_id = proxy.i64_field("alterId").unwrap_or(0);
    o.insert(
        "alter_id".into(),
        mapped_val("alterId", &json!(alt_id), "type", "rename"),
    );
}
...
fn add_vless_origins(o: &mut Map<String, Value>, proxy: &ClashProxy, outbound: &Value) {
        ...
        && let Some(flow) = proxy.str_field("flow")
    {
        o.insert(
            "flow".into(),
            mapped_val("flow", &json!(flow), "type", "direct"),
        );
    }
}
...
fn add_ss_origins(o: &mut Map<String, Value>, proxy: &ClashProxy, outbound: &Value) {
    ...
    if out_type == "shadowtls" {
        ...
        o.insert(
            "type".into(),
            mapped_val("plugin", &json!("shadow-tls"), "type", "convert"),
        );
        if outbound.get("password").is_some() {
            o.insert(
                "password".into(),
                mapped("plugin-opts.password", "type", "extract"),
            );
        }
        if outbound.get("version").is_some() {
            o.insert(
                "version".into(),
                mapped("plugin-opts.version", "type", "extract"),
            );
        }
        ...
        if let Some(v) = proxy.extra.get("password") {
            o.insert(
                "password".into(),
                mapped_val("password", v, "type", "direct"),
            );
        }
        ...
        if outbound.get("plugin").is_some() {
            if let Some(plugin) = proxy.str_field("plugin") {
                o.insert(
                    "plugin".into(),
                    mapped_val("plugin", &json!(plugin), "type", "convert"),
                );
            }
            if outbound.get("plugin_opts").is_some() {
                o.insert(
                    "plugin_opts".into(),
                    mapped("plugin-opts", "type", "convert"),
                );
            }
        }
    }
...
fn add_hysteria2_origins(o: &mut Map<String, Value>, proxy: &ClashProxy, outbound: &Value) {
        ...
        && let Some(up) = proxy.str_field("up")
    {
        o.insert(
            "up_mbps".into(),
            mapped_val("up", &json!(up), "type", "convert"),
        );
    }
        ...
        && let Some(down) = proxy.str_field("down")
    {
        o.insert(
            "down_mbps".into(),
            mapped_val("down", &json!(down), "type", "convert"),
        );
    }
    ...
    if outbound.get("tls").is_some() && !o.contains_key("tls") {
        ...
        o.insert(
            "tls.enabled".into(),
            generated("tls", "hysteria2_requires_tls"),
        );
...
fn add_hysteria_origins(o: &mut Map<String, Value>, proxy: &ClashProxy, outbound: &Value) {
    ...
    if let Some(down) = proxy.str_field("down") {
        o.insert(
            "down".into(),
            mapped_val("down", &json!(down), "type", "direct"),
        );
    }
        ...
        && let Some(obfs) = proxy.str_field("obfs")
    {
        o.insert(
            "obfs".into(),
            mapped_val("obfs", &json!(obfs), "type", "direct"),
        );
    }
        ...
        && let Some(auth) = proxy.str_field("auth-str")
    {
        o.insert(
            "auth_str".into(),
            mapped_val("auth-str", &json!(auth), "type", "rename"),
        );
    }
    ...
    if outbound.get("tls").is_some() && !o.contains_key("tls") {
        o.insert(
            "tls".into(),
            container("tls", &["sni", "alpn", "skip-cert-verify"]),
        );
        o.insert(
            "tls.enabled".into(),
            generated("tls", "hysteria_requires_tls"),
        );
        if let Some(sni) = proxy.str_field("sni") {
            o.insert(
                "tls.server_name".into(),
                mapped_val("sni", &json!(sni), "tls", "rename"),
            );
        } else {
            o.insert(
                "tls.server_name".into(),
...
fn add_tuic_origins(o: &mut Map<String, Value>, proxy: &ClashProxy, outbound: &Value) {
    ...
    if outbound.get("zero_rtt_handshake").is_some() {
        o.insert(
            "zero_rtt_handshake".into(),
            mapped("reduce-rtt", "type", "rename"),
        );
    }
    ...
    if outbound.get("udp_over_stream").is_some() {
        o.insert(
            "udp_over_stream".into(),
            mapped("udp-over-stream", "type", "rename"),
        );
    }
    ...
    if outbound.get("tls").is_some() && !o.contains_key("tls") {
        o.insert(
            "tls".into(),
            container("tls", &["sni", "skip-cert-verify", "alpn"]),
        );
...
fn add_http_origins(o: &mut Map<String, Value>, proxy: &ClashProxy) {
    if let Some(u) = proxy.str_field("username") {
        o.insert(
            "username".into(),
            mapped_val("username", &json!(u), "type", "direct"),
        );
    }
    if let Some(p) = proxy.str_field("password") {
        o.insert(
            "password".into(),
            mapped_val("password", &json!(p), "type", "direct"),
        );
    }
}
