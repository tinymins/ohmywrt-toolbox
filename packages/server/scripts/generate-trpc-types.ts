  ...
  for (const sourceFile of routers) {
    ...
    for (const classDecl of classes) {
      ...
      for (const method of classDecl.getMethods()) {
        ...
        const subscriptionMeta = getDecoratorArgObject("Subscription", method);
        const decorator = queryMeta
          ...
          : mutationMeta
            ...
            : subscriptionMeta
              ? "subscription"
              : null;
        const meta = queryMeta ?? mutationMeta ?? subscriptionMeta;
        ...
        if (decorator === "subscription") {
          // For subscriptions, don't use .output() — tRPC v11 infers
          // the yield type from the AsyncGenerator return type annotation
          const yieldType = outputValue
            ? `z.infer<typeof ${outputValue}>`
            : "any";
          parts.push(
            `subscription(async function* (opts): AsyncGenerator<${yieldType}> { yield "PLACEHOLDER_DO_NOT_REMOVE" as any; })`,
          );
        } else {
