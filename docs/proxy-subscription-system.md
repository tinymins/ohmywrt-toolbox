...
| 格式 | 校验方式 | 工具 |
|------|----------|------|
| sing-box / sing-box-macos / sing-box-windows | 二进制校验 `sing-box check -c` | sing-box v1.11.x |
| sing-box-v12 / v13（含 macOS / Windows） | 二进制校验（v1.12+ / v1.13+） | 对应 sing-box 版本 |
...
```bash
unshare --user --net -- sing-box check -c /tmp/validate_xxx.json
```

| 措施 | 防范 |
|------|------|
| `unshare --user --net` | 用户+网络命名空间隔离，无需 CAP_SYS_ADMIN，即使 RCE 也无法发起网络连接 |
| Tokio 5 秒超时 | 防止挂死、死循环，不依赖 GNU timeout |
...
3. 直接执行（仅当 `ALLOW_INSECURE_VALIDATION=true`，**生产环境禁止**；macOS 本地调试会走这个降级）

如果 `unshare` 不可用（权限不足），行为由 `ALLOW_INSECURE_VALIDATION` 环境变量控制：

| 环境 | 值 | 行为 |
|------|------|------|
| 开发 | `true` | 降级为 `sing-box check` + Tokio 5 秒超时（无网络隔离） |
...
| `DATA_LOCAL_PATH/vendors/sing-box-v13/` | sing-box v1.13.14 | sing-box-v13 格式校验 |
| `DATA_LOCAL_PATH/vendors/mihomo/` | mihomo v1.19.22 | clash/clash-meta 格式（预留） |

- 脚本通过 `.version` 标记文件实现幂等（已存在且版本匹配则跳过下载）
- 脚本按当前系统下载 Linux 或 macOS 的 amd64/arm64 二进制；`.version` 会包含 OS/arch，避免跨系统复用错误二进制
